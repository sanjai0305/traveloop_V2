...
const TripBudget = () => {
  ...
  const remainingBudget = useMemo(() => {
    if (activeBudget && typeof activeBudget.remainingBudget === "number" && !isNaN(activeBudget.remainingBudget)) {
      return activeBudget.remainingBudget;
    }
    return budgetLimit - totalSpent;
  }, [activeBudget, budgetLimit, totalSpent]);
  ...
  // Safe calculation for active budget utilization percentage and remaining budget
  const utilizationPercentage = useMemo(() => {
    if (!activeBudget) return 0;
    if (typeof activeBudget.utilizationPercentage === "number" && !isNaN(activeBudget.utilizationPercentage)) {
      return activeBudget.utilizationPercentage;
    }
    const total = Number(activeBudget.totalBudget) || 0;
    if (total <= 0) return 0;
    const spent = Number(activeBudget.actualExpense) || Number(activeBudget.plannedExpense) || 0;
    ...
    <MainLayout>
      <div className="px-4 pt-4 pb-24">
        <PageHeader
          ...
          actions={
            <div className="relative">
              <select
                ...
                className="px-3.5 py-2 rounded-xl border border-slate-200 text-xs font-bold bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 outline-none shadow-xs"
              >
          ...
          badge={
            pendingSync ? (
              <span className="px-2 py-1 rounded-full text-[10px] font-extrabold bg-amber-500/10 text-amber-500 animate-pulse">
                Pending Sync
              </span>
            ) : null
          }
        ...
        {/* Empty Budget State Callout */}
        {!budgetsLoading && (!budgets || budgets.length === 0 || !activeBudget) && (
          <div className="p-8 rounded-3xl bg-white dark:bg-slate-900 border-2 border-dashed border-slate-200 dark:border-slate-800 text-center space-y-4 my-6 shadow-sm">
            <div className="w-16 h-16 rounded-full bg-teal-50 dark:bg-teal-950/40 text-teal-500 flex items-center justify-center text-3xl mx-auto ring-8 ring-teal-500/10">
              💰
            </div>
            <div className="space-y-1.5 max-w-md mx-auto">
              <h3 className="text-lg font-black text-slate-900 dark:text-white">
                No budget has been created for this trip yet.
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400 font-medium leading-relaxed">
                Set up a custom budget to track transportation, accommodation, food, activities, and daily expenses for {trip?.title || "your trip"}.
              </p>
            </div>
            {!isViewer && (
              <button
                onClick={() => setIsCreateBudgetOpen(true)}
                className="bg-gradient-to-r from-teal-500 to-emerald-500 text-white font-extrabold text-xs px-6 py-3 rounded-full hover:scale-105 active:scale-95 shadow-lg shadow-teal-500/30 transition-all inline-flex items-center gap-2"
              >
          ...
          <div className="premium-card p-5 mb-5 space-y-4 text-left">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
              ...
              <div className="p-3 rounded-2xl bg-slate-50 border border-slate-100 flex flex-col justify-between">
                ...
                <span className={`text-sm font-extrabold ${remainingBudget >= 0 ? "text-emerald-600" : "text-rose-600"}`}>
                  {formatCurrency(remainingBudget).split(" (")[0]}
                </span>
              </div>
