...
// Premium Legal Consent & Onboarding Screen - Agent Portal
...
const LegalConsent: React.FC = () => {
  ...
  useEffect(() => {
    if (agent?.mobile || agent?.phone) {
      const p = agent.mobile || agent.phone || "";
      const digits = p.replace(/\D/g, "");
      if (digits.length === 12 && digits.startsWith("91")) {
        setMobile(digits.slice(2));
      } else {
        setMobile(digits);
      }
    }
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (updatedAgent?.mobileVerified) {
  ...
  const handleSendMobileOtp = async () => {
    ...
    try {
      ...
      console.error("[LegalConsent] Mobile OTP send error:", err);
  ...
  const handleVerifyMobileOtp = async () => {
    ...
    try {
      ...
      if (response.data?.success) {
        setAuth(localStorage.getItem("agentToken") || "", response.data.agent);
      ...
      console.error("[LegalConsent] Mobile OTP verify error:", err);
      alert(err.message || "Invalid OTP code entered.");
    } finally {
                ...
                {otpSent && (
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={otpVerifying || mobileOtp.length < 6}
                      className={`w-full py-4 rounded-xl font-bold text-sm uppercase tracking-wider transition-all flex items-center justify-center gap-2 ${
                        mobileOtp.length === 6 && !otpVerifying
                          ? "bg-gradient-to-r from-cyan-500 to-teal-500 hover:from-cyan-400 hover:to-teal-400 text-slate-950 shadow-lg shadow-cyan-500/25 active:scale-[0.98]"
                          : "bg-slate-900 text-slate-600 border border-slate-800/80 cursor-not-allowed"
                      }`}
                    >
