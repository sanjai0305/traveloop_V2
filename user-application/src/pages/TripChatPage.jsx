      ...
      <div className="w-full h-full flex flex-col overflow-hidden bg-slate-100 font-sans">
        ...
        <div className="flex-1 w-full max-w-[1440px] mx-auto flex overflow-hidden bg-white shadow-xl">
          ...
          <div className="flex-1 flex flex-col justify-between bg-white min-w-0 h-full overflow-hidden">
            ...
            <div className="p-4 border-t border-slate-100 bg-white shrink-0 z-20 space-y-2">
              {/* Driver Announcement Quick Templates */}
              <div className="flex items-center gap-1.5 overflow-x-auto hide-scrollbar pb-1">
                <span className="text-[10px] font-black text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full shrink-0">
                  📢 Quick Announcements:
                </span>
                {[
                  "I have reached pickup point.",
                  "Bus departs in 10 minutes.",
                  "Please keep your QR ready.",
                  "Seat allocation updated."
                ].map((txt, idx) => (
                  <button
                    key={idx}
                    type="button"
                    onClick={() => setInput(`📢 Announcement: ${txt}`)}
                    className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-slate-100 hover:bg-slate-200 text-slate-700 whitespace-nowrap transition-all border border-slate-200 shrink-0"
                  >
                    {txt}
                  </button>
                ))}
