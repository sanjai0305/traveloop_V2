      ...
      >
        <div className="space-y-4">
          <p className="text-[10px] text-slate-400 dark:text-slate-500 font-semibold">
            Select a day schedule for: <span className="font-bold text-slate-700 dark:text-slate-200">{selectedAct?.title || selectedAct?.name}</span>
          </p>
          <div className="space-y-2 mb-4">
            {Array.from({ length: daysCount }, (_, i) => i + 1).map(day => (
              <button key={day} onClick={() => handleAddConfirm(day)} disabled={adding}
                className="w-full flex items-center justify-between p-3.5 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 hover:bg-teal-50/20 hover:border-teal-300 text-xs font-bold text-slate-700 dark:text-slate-200 active:scale-98 transition-all">
